    ...
    public String selectValue(String query) throws SQLException {
        ...
        if (rSet.next()){
            if (rSet.getObject(1)!=null){
                ...
                if(meta.getColumnType(1)==93){ //93 - it's a type in DB where time is written
                    value=value.substring(0,value.length()-2); //cutting of milliseconds
                }
            }
        }
        ...
        value=value.trim(); //deletion of spaces
        return value;
    }
